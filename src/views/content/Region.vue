...
import '@/lz-helpers';
import { PORTALDEV_URL } from '@/util';
...
/**
 * Internal content of the region route, once data is availabile.
  */
...
export default {
  ...
  props: [
    'region_data',
    'chrom', 'start', 'end',
    'y_field', 'gene_id', 'tissue', 'extra_genes', 'extra_tissues',
  ],
  data() {
    return {
  ...
  beforeMount() {
    const { region_data: data, y_field } = this;
  ...
  methods: {
    ...
    addTrack(type, track_id) {
      ...
      this.$emit('add_track', type, track_id);
    },
  },
  watch: {
    y_field() {
