...
filelist = sorted(glob.glob(os.path.join(csdir, "chr*.ge.credible_set.tsv.gz")), key=chrnum)

def parseline(line):
    try:
        (study, tissue, phenotype_id, variant_id, chrom, pos, ref, alt, cs_id, cs_index, finemapped_region, pip, z, cs_min_r2, cs_avg_r2, cs_size, posterior_mean, posterior_sd, cs_log10bf) = line.decode('utf-8').rstrip('\n').split()
        return([float(pip), study, tissue, phenotype_id, chrom, int(pos), ref, alt, cs_index, int(cs_size)])
    except:
        return([0.0, "NO_STUDY", "NO_TISSUE", "NO_GENE", "NO_CHR", 0, "NO_REF", "NO_ALT", "L0", 0])
...
with conn:
    ...
    cursor.execute("CREATE TABLE sig (pip REAL, study TEXT, tissue TEXT, gene_id TEXT, chrom TEXT, pos INTEGER, ref TEXT, alt TEXT, cs_index TEXT, cs_size INTEGER)")
        ...
        with gzip.open(infile) as f:
            ...
            for line in f:
                ...
                if bestList[4:8] == currentList[4:8]:
                    ...
                    cursor.execute("INSERT INTO sig VALUES (?,?,?,?,?,?,?,?,?,?)", tuple(bestList))
                    bestList = currentList
            cursor.execute("INSERT INTO sig VALUES (?,?,?,?,?,?,?,?,?,?)", tuple(bestList))
