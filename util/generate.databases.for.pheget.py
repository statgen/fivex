...
parser = argparse.ArgumentParser(
    description="Generates an sqlite3 database which can be used to match gene names to gene symbols and genomic position ranges"
)  # noqa
parser.add_argument(
    "-i",
    "--infile",
...
parser.add_argument(
    ...
    metavar="OUTDB",
    type=str,
    default="../data/gene.chrom.pos.lookup.sqlite3.db",
...
parser.add_argument(
    ...
    metavar="OUTPICKLE",
    type=str,
    default="../data/gene.symbol.pickle",
...
c = conn.cursor()
c.execute(
    "CREATE TABLE IF NOT EXISTS genelookup(gene_id text PRIMARY KEY, symbol text, chrom text, startpos int, endpos int, type text)"
)  # noqa

with gzip.open(args.infile) as f:
    for line in f:
        (chrom, hav, temptype, begin, end, id1, strand, id2, infoFields) = (
        ...
        c.execute(
            "INSERT OR REPLACE INTO genelookup VALUES (?,?,?,?,?,?)",
            (name, symbol, chrom, begin, end, dataType),
        )
conn.commit()
conn.close()
