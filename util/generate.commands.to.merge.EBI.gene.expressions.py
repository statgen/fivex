...
chrDict = {'1':249, '2':243, '3':199, '4':191, '5':182, '6':171, '7':160, '8':146, '9':139, '10':134, '11':136, '12':134, '13':115, '14':108, '15':102, '16':91, '17':84, '18':81, '19':59, '20':65, '21':47, '22':51}#, 'X':157}
...
outputCommandFile = os.path.join(outDir, "extract.and.tabix.gene_expression.sh")
...
with open(outputCommandFile, "w") as w:
    for chrom in chrDict:
        temp = subprocess.run(['mkdir', '-p', os.path.join(geDirectory, chrom)])
        for i in range(chrDict[chrom]):
            end = (i + 1) * 1000000
            start = (i * 1000000) + 1
            w.write(f'python3 {scriptPath} {indexFile} {chrom} {start} {end} {geDirectory}/{chrom}/all.EBI.ge.data.chr{chrom}.{start}-{end}.tsv.gz\n')
    for chrom in chrDict:
        for i in range(chrDict[chrom]):
            end = (i + 1) * 1000000
            start = (i * 1000000) + 1
            w.write(f"tabix -s 4 -b 5 -e 5 {geDirectory}/{chrom}/all.EBI.ge.data.chr{chrom}.{start}-{end}.tsv.gz\n")
