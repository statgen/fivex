...
with open("run.extract.Makefile","w") as w:
    ...
    w.write(tempdir+"all_chr.All_Tissues.sorted.txt.gz:")
    for i in range(1,23) + ["X"]:
        infile = tempdir+"chr"+str(i)+".All_Tissues.sorted.txt.gz"
        w.write(" " + infile)
    infile = tempdir+"chr1.All_Tissues.sorted.txt.gz"
    w.write("\n\t(zcat " + infile)
    for i in range(2,23) + ["X"]:
        infile = tempdir+"chr"+str(i)+".All_Tissues.sorted.txt.gz"
        w.write(" ; zcat " + infile + " | tail -n +2")
    w.write(") | bgzip -c > " + tempdir + "all_chr.All_Tissues.sorted.txt.gz\n\n")

    # Create sorted chromosome-specific, all-tissues temporary data files from chromosome-specific, tissue-specific data files
    for i in range(1,23) + ["X"]:
        outfile = tempdir + "chr" + str(i) + ".All_Tissues.sorted.txt.gz"
        infileList = list()
        for tissue in tissueList:
    ...
    for tissue in tissueList:
        outfile = tempdir+tissue+".sorted.txt.gz"
        infileList = list()
        for i in range(1,23) + ['X']:
